            ...
            this.state.data["data"].forEach(function(element) {
                employeesRows.push(
                    <tr key={"employee-" + element.id} className="border-t border-b border-white border-opacity-40">
                        ...
                        <td className="px-5 text-center py-4">{User.isOwnerOfCompany() && <button id={"kick-employee-button-" + element.id} className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded disabled:opacity-50" onClick={() => thisComponent.kickEmployee(element.id)}>Kick</button>}</td>
                    </tr>
                );
            });
        }
